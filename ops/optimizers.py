...
def get_optimizer(
    ...
    if dtype == tf.float32 or dtype == tf.bfloat16:
        ...
        raise NotImplementedError('Need to figure out eps for dtype: %s' % dtype)
    with tf.control_dependencies(update_ops):
        if optimizer == 'adam':
            optim = lambda x: tf.train.AdamOptimizer(x, epsilon=eps)
        elif optimizer == 'nadam':
            optim = lambda x: tf.contrib.opt.NadamOptimizer(x, epsilon=eps)
