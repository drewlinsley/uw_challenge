...
def get_grad_plots(
    ...
    gmax, gmin = 10, -10  # 8, 0 # np.max([nze, nzi, ze, zi])
    smax, smin = 10, -10  # 4, -4
    if ran is None:
        ran = range(len(e))
    count = 0
    for im in ran:
        ...
        plt.title('$H^{(1)}$')
        im1 = ax1.imshow(gaussian(nzi.squeeze(), sig, preserve_range=True), cmap='PuOr_r', vmin=smin, vmax=smax)
        prep_subplot(f=f, im=im1, ax=ax1)

        ax2 = plt.subplot(2, 4, 3)
        plt.title('$H^{(2)}$')
        im2 = ax2.imshow(gaussian(nze.squeeze(), sig, preserve_range=True), cmap='PuOr_r', vmin=smin, vmax=smax)
        prep_subplot(f=f, im=im2, ax=ax2)

        ax3 = plt.subplot(2, 4, 4)
        ndif = gaussian((nze.squeeze()), sig, preserve_range=True) + gaussian((nzi.squeeze()), sig, preserve_range=True)
        plt.title('$H^{(2)} + H^{(1)}, max=%s$' % np.around(ndif.max(), 2))
        im3 = ax3.imshow((ndif.squeeze()), cmap='RdBu_r', vmin=gmin, vmax=gmax)
        prep_subplot(f=f, im=im3, ax=ax3, ticks=[gmin, gmax])
        plt.subplot(2, 4, 1)
        plt.title('Decision: %s' % nlogits[im])
        plt.imshow(nims[im].squeeze(), cmap='Greys_r')
        plt.axis('off')
        ...
        plt.title('$H^{(1)}$')
        im4 = ax4.imshow(gaussian(zi.squeeze(), sig, preserve_range=True), cmap='PuOr_r', vmin=smin, vmax=smax)
        prep_subplot(f=f, im=im4, ax=ax4)
        ax5 = plt.subplot(2, 4, 7)
        plt.title('$H^{(2)}$')
        im5 = ax5.imshow(gaussian(ze.squeeze(), sig, preserve_range=True), cmap='PuOr_r', vmin=smin, vmax=smax)
        prep_subplot(f=f, im=im5, ax=ax5)
        ax6 = plt.subplot(2, 4, 8)
        dif = gaussian((ze.squeeze()), sig, preserve_range=True) + gaussian((zi.squeeze()), sig, preserve_range=True)
        plt.title('$H^{(2)} + H^{(1)}, max=%s$' % np.around(dif.max(), 2))
        im6 = ax6.imshow((dif.squeeze()), cmap='RdBu_r', vmin=gmin, vmax=gmax)
        prep_subplot(f=f, im=im6, ax=ax6, ticks=[gmin, gmax])
        plt.subplot(2, 4, 5)
        plt.title('Decision: %s' % logits[im])
        plt.imshow(ims[im].squeeze(), cmap='Greys_r')
        plt.axis('off')
        if save_figs:
