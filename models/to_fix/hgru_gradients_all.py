...
def main(gpu_device='/gpu:0', cpu_device='/cpu:0'):
    ...
    # Derive gradients
    exc_train_gradients = [
        tf.gradients(x, train_images) for x in exc_train_activities]
    exc_val_gradients = [
        tf.gradients(x, val_images) for x in exc_val_activities]
    inh_train_gradients = [
        tf.gradients(x, train_images) for x in inh_train_activities]
    inh_val_gradients = [
        tf.gradients(x, val_images) for x in inh_val_activities]
