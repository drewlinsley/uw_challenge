...
class hGRU(object):
    ...
    def prepare_tensors(self):
        ...
        if self.force_horizontal:
            ...
            if self.time_homunculus:
                homu_init = tf.zeros(self.timesteps) + 0.5
            else:
                homu_init = tf.zeros([]) + 0.5
        ...
        for idx, hgru_id in enumerate(self.hgru_ids):
            ...
            with tf.variable_scope(
                ...
                if self.symmetric_weights and self.symmetric_inits:
                    iv = self.symmetric_init(getattr(self, '%s_horizontal_kernels_inh_%s' % (self.symm_k_tag, layer)))
                    ev = self.symmetric_init(getattr(self, '%s_horizontal_kernels_exc_%s' % (self.symm_k_tag, layer)))
                    setattr(self, '%s_horizontal_kernels_inh_%s' % (self.symm_k_tag, layer), iv)
                    setattr(self, '%s_horizontal_kernels_exc_%s' % (self.symm_k_tag, layer), ev)
                setattr(
                    self,
    ...
    def td_ops(self, activity, l1_h2, i0):
        ...
        if self.time_homunculus:
             homunculus = tf.maximum(tf.minimum(self.homunculus[i0], 1.), 0.)
        else:
             homunculus = tf.maximum(tf.minimum(self.homunculus, 1.), 0.)
    ...
    def full(self, i0, x, l1_h2, l2_h2, fb_act_1):
        ...
        # Add Upsamples
        if self.readout == 'l2' and i0 == (self.timesteps - 1):
