...
class hGRU(object):
    ...
    def prepare_tensors(self):
        ...
        for idx, hgru_id in enumerate(self.hgru_ids):
            ...
            with tf.variable_scope(
                ...
                setattr(
                    self,
                    '%s_gain_kernels_W_%s' % (self.symm_g_tag, layer),
                    tf.get_variable(
                        name='%s_%s_gain_W' % (self.symm_g_tag, self.layer_name),
                        dtype=self.dtype,
                        trainable=True,
                        shape=self.g_shape,
                        initializer=tf.initializers.variance_scaling()))
                setattr(
                    self,
                    '%s_mix_kernels_W_%s' % (self.symm_g_tag, layer),
                    tf.get_variable(
                        name='%s_%s_mix_W' % (self.symm_g_tag, self.layer_name),
                        dtype=self.dtype,
                        trainable=True,
                        shape=self.m_shape,
                        initializer=tf.initializers.variance_scaling()))
                setattr(
                    self,
                    '%s_gain_kernels_U_%s' % (self.symm_g_tag, layer),
                    tf.get_variable(
                        name='%s_%s_gain_U' % (self.symm_g_tag, self.layer_name),
                        dtype=self.dtype,
                        trainable=True,
                        shape=self.g_shape,
                        initializer=tf.initializers.variance_scaling()))
                setattr(
                    self,
                    '%s_mix_kernels_U_%s' % (self.symm_g_tag, layer),
                    tf.get_variable(
                        name='%s_%s_mix_U' % (self.symm_g_tag, self.layer_name),
                        dtype=self.dtype,
                        trainable=True,
                        shape=self.m_shape,
                        initializer=tf.initializers.variance_scaling()))
    ...
    def hgru_ops(self, i0, x, h2, layer):
        ...
        # Gather the gates
        gain_kernels_W = getattr(self, '%s_gain_kernels_W_%s' % (
            self.symm_g_tag, layer))
        mix_kernels_W = getattr(self, '%s_mix_kernels_W_%s' % (
            self.symm_g_tag, layer))
        gain_kernels_U = getattr(self, '%s_gain_kernels_U_%s' % (
            self.symm_g_tag, layer))
        mix_kernels_U = getattr(self, '%s_mix_kernels_U_%s' % (
            self.symm_g_tag, layer))
        gain_bias = getattr(self, 'gain_bias_%s' % layer)
        mix_bias = getattr(self, 'mix_bias_%s' % layer)

        # Do gain convolutions
        g1_W = self.conv_2d_op(
            data=x,
            weights=gain_kernels_W,
            symmetric_weights=self.symmetric_gate_weights,
            dilations=self.dilations[layer_idx])
        with tf.variable_scope(
                '%s/g1_W_bn' % var_scope,
                reuse=self.scope_reuse) as scope:
            g1_W = tf.contrib.layers.batch_norm(
                inputs=g1_W,
                scale=True,
                center=False,
                fused=True,
        ...
        g1_U = self.conv_2d_op(
            data=h2,
            weights=gain_kernels_U,
            symmetric_weights=self.symmetric_gate_weights,
            dilations=self.dilations[layer_idx])
        with tf.variable_scope(
                '%s/g1_U_bn' % var_scope,
                reuse=self.scope_reuse) as scope:
            g1_U = tf.contrib.layers.batch_norm(
                inputs=g1_U,
                scale=True,
                center=False,
                fused=True,
                ...
                reuse=self.reuse,
                is_training=self.train)
        r = tf.sigmoid(g1_W + g1_U + gain_bias)

        # Do mix convolutions
        m1_W = self.conv_2d_op(
            data=x,
            weights=mix_kernels_W,
            symmetric_weights=self.symmetric_gate_weights,
            dilations=self.dilations[layer_idx])
        with tf.variable_scope(
                '%s/m1_W_bn' % var_scope,
                reuse=self.scope_reuse) as scope:
            m1_W = tf.contrib.layers.batch_norm(
                inputs=m1_W,
                scale=True,
                center=False,
                fused=True,
                ...
                reuse=self.reuse,
                is_training=self.train)
        m1_U = self.conv_2d_op(
            data=h2,
            weights=mix_kernels_U,
            symmetric_weights=self.symmetric_gate_weights,
            dilations=self.dilations[layer_idx])
        with tf.variable_scope(
                '%s/m1_U_bn' % var_scope,
                reuse=self.scope_reuse) as scope:
            m1_U = tf.contrib.layers.batch_norm(
                inputs=m1_U,
                scale=True,
                center=False,
                fused=True,
                ...
                reuse=self.reuse,
                is_training=self.train)
        z = tf.sigmoid(m1_W + m1_U + mix_bias)
        ...
        # Calculate candidate
        h2_tilda = self.ff_nl(c1_W + c1_U + activity_bias) 

        # Integrate
        h2 = (1 - z) * h2 + z * h2_tilda
        return h2
